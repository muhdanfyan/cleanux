...
Cleanux is a macOS system cleanup and optimization tool with shell and Go components. It performs file cleanup, app protection checks, and maintenance tasks, so safety rules matter more than speed.
...
- `mo completion`, `mo touchid`, `mo update`, and `mo remove` manage shell integration, sudo auth convenience, updates, and uninstalling Cleanux itself.
...
```bash
gh api graphql -f query='{user(login:"muhdanfyan"){sponsorshipsAsMaintainer(first:30, orderBy:{field:CREATED_AT, direction:DESC}){nodes{sponsorEntity{... on User{login} ... on Organization{login}}}}}}'
```
...
```bash
RELEASE_ID=$(gh api repos/muhdanfyan/Cleanux/releases/tags/V<version> --jq '.id')
for r in +1 laugh hooray heart rocket eyes; do
  gh api "repos/muhdanfyan/Cleanux/releases/$RELEASE_ID/reactions" -X POST -f content="$r" --silent
done
```
